...
$(document).ready(function () {
    var testimonialDataTable = $('#testimonialDataTable').DataTable({
        ...
        buttons: Helper.datatableButtons([0, 1, 2, 5], 'testimonial'),
        ...
        columns: [{
            ...
            render: function (data, row, type, col) {
                var pageInfo = testimonialDataTable.page.info();
                return (col.row + 1) + pageInfo.start;
            }
        },
        {
            title: Helper.trans('root.general_settings_testimonial.title'),
            name: 'title',
            data: 'title'
        },
        {
            title: Helper.trans('root.general_settings_testimonial.description'),
            name: 'description',
            data: 'description',
            width: '300px',
        },
        {
            title: Helper.trans('root.general_settings_testimonial.author_name'),
            name: 'author_name',
            data: 'author_name',
            width: '100px',
        },
        {
            title: Helper.trans('root.general_settings_testimonial.author_company_name'),
            name: 'author_company_name',
            data: 'author_company_name',
            width: '100px',
        },
        {
            title: Helper.trans('root.general_settings_testimonial.number_of_star'),
            name: 'number_of_star',
            data: 'number_of_star',
            width: '80px',
        },
        {
            title: Helper.trans('root.general_settings_testimonial.image_path'),
            name: 'image_path',
            data: 'image_path',
            width: '300px',
        ...
        },
        {
            ...
            render: function (data, type, row, col) {
                var url_edit = `admin/general-settings/testimonial/${data}/edit`;
                var url = `admin/general-settings/testimonial/${data}`;
                let returnData = ``;
                ...
                if (Helper.isShow('drivers.destroy')) {
                    returnData += `<a data-val="${data}" class="btn btn-xs btn-danger waves-effect m-r-5 testimonialDelete"
        ...
        ajax: {
            url: Helper.siteUrl("admin/general-settings/testimonial"),
        },
        language: {
            paginate: {
                next: '&#8594;', // or '→'
    ...
    $(document).on('click', '.testimonialDelete', function () {
        ...
        Swal.fire(config).then(function (result) {
            if (result.value === true) {
                Helper.delete('admin/general-settings/testimonial/' + el.attr('data-val'), '#testimonialDataTable');
            }
        });
    });
    ...
    $(document).on('click', '.custome-switch', function () {
        var el = $(this);
        Helper.updateStatus('admin/general-settings/testimonial/update-status/' + el.attr('data-val'));
    });

    // $("#testimonialDataTable tbody").sortable({
    //     items: "tr",
    //     cursor: 'move',
    //     opacity: 0.6,
    //     update: function() {
    //         sendOrderToServer('#testimonialDataTable', 'admin/slider-sortable', testimonialDataTable.page
    //             .info().page);
    //     }
    // });
