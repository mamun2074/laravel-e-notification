...
$(document).ready(function () {
    var appartmentGalleryDataTable = $('#appartmentGalleryDataTable').DataTable({
        ...
        columns: [{
            ...
            render: function (data, row, type, col) {
                var pageInfo = appartmentGalleryDataTable.page.info();
                return (col.row + 1) + pageInfo.start;
            }
        },
        {
            title: Helper.trans('root.appartment_manage_appartment_galaries.appartment_gallery_type_id'),
            name: 'appartment_gallery_type_name',
            data: 'appartment_gallery_type_name',
            render: (data) => data
        },
        {
            title: Helper.trans('root.appartment_manage_appartment_galaries.appartment_id'),
            name: 'appartment_name',
            data: 'appartment_name',
            render: (data) => data
        },
        {
            title: Helper.trans('root.appartment_manage_appartment_galaries.photo_path'),
            name: 'photo_path',
            data: 'photo_path',
            render: function (data, type, row, col) {
                return (data) ? `<img width="50px" src="${Helper.getStorageUrl(data)}" alt="">` : '';
            }
        },
        {
        ...
        },
        {
            ...
            render: function (data, type, row, col) {
                var url_edit = `admin/appartment-manage/appartment-gallery/${data}/edit`;
                var url = `admin/appartment-manage/appartment-gallery/${data}`;
                let returnData = ``;
                if (Helper.isShow('appartment_galleries.edit')) {
                ...
                if (Helper.isShow('appartment_galleries.destroy')) {
                    returnData += `<a data-val="${data}" class="btn btn-xs btn-danger waves-effect m-r-5 appartmentGalleryDelete"
        ...
        ajax: {
            url: Helper.siteUrl("admin/appartment-manage/appartment-gallery"),
        },
        language: {
            paginate: {
                next: '&#8594;', // or '→'
    ...
    $(document).on('click', '.appartmentGalleryDelete', function () {
        var el = $(this);
        let config = Object.assign(Helper.swalConfig());
        Swal.fire(config).then(function (result) {
            if (result.value === true) {
                Helper.delete('admin/appartment-manage/appartment-gallery/' + el.attr('data-val'), '#appartmentGalleryDataTable');
            }
        });
    });
    ...
    $(document).on('click', '.custome-switch', function () {
        var el = $(this);
        Helper.updateStatus('admin/appartment-manage/appartment-gallery/update-status/' + el.attr('data-val'));
    });

    // $("#appartmentGalleryDataTable tbody").sortable({
    //     items: "tr",
    //     cursor: 'move',
    //     opacity: 0.6,
    //     update: function() {
    //         sendOrderToServer('#appartmentGalleryDataTable', 'admin/slider-sortable', appartmentGalleryDataTable.page
    //             .info().page);
    //     }
    // });
