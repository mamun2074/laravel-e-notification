...
$(document).ready(function () {
    var appartmentGalleryTypeDataTable = $('#appartmentGalleryTypeDataTable').DataTable({
        ...
        columns: [{
            ...
            render: function (data, row, type, col) {
                var pageInfo = appartmentGalleryTypeDataTable.page.info();
                return (col.row + 1) + pageInfo.start;
            }
        },
        {
            title: Helper.trans('root.appartment_manage_appartment_gallery_type.name'),
            name: 'name',
            data: 'name'
        },
        {
            title: Helper.trans('root.appartment_manage_appartment_gallery_type.description'),
            name: 'description',
            data: 'description'
        },
        {
        ...
        },
        {
            ...
            render: function (data, type, row, col) {
                var url_edit = `admin/appartment-manage/appartment-gallery-type/${data}/edit`;
                var url = `admin/appartment-manage/appartment-gallery-type/${data}`;
                let returnData = ``;
                if (Helper.isShow('appartment_gallery_types.edit')) {
                ...
                if (Helper.isShow('appartment_gallery_types.destroy')) {
                    returnData += `<a data-val="${data}" class="btn btn-xs btn-danger waves-effect m-r-5 appartmentGalleryTypeDelete"
        ...
        ajax: {
            url: Helper.siteUrl("admin/appartment-manage/appartment-gallery-type"),
        },
        language: {
            paginate: {
                next: '&#8594;', // or '→'
    ...
    $(document).on('click', '.appartmentGalleryTypeDelete', function () {
        var el = $(this);
        let config = Object.assign(Helper.swalConfig());
        Swal.fire(config).then(function (result) {
            if (result.value === true) {
                Helper.delete('admin/appartment-manage/appartment-gallery-type/' + el.attr('data-val'), '#appartmentGalleryTypeDataTable');
            }
        });
    });
    ...
    $(document).on('click', '.custome-switch', function () {
        var el = $(this);
        Helper.updateStatus('admin/appartment-manage/appartment-gallery-type/update-status/' + el.attr('data-val'));
    });

    // $("#appartmentGalleryTypeDataTable tbody").sortable({
    //     items: "tr",
    //     cursor: 'move',
    //     opacity: 0.6,
    //     update: function() {
    //         sendOrderToServer('#appartmentGalleryTypeDataTable', 'admin/slider-sortable', appartmentGalleryTypeDataTable.page
    //             .info().page);
    //     }
    // });
