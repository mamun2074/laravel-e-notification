...
$(document).ready(function () {
    var appartmentFacilityDataTable = $('#appartmentFacilityDataTable').DataTable({
        ...
        columns: [{
            ...
            render: function (data, row, type, col) {
                var pageInfo = appartmentFacilityDataTable.page.info();
                return (col.row + 1) + pageInfo.start;
            }
        },
        {
            title: Helper.trans('root.appartment_manage_appartment_facility.appartment_id'),
            name: 'appartment_id',
            data: 'appartment_id'
        },
        {
            title: Helper.trans('root.appartment_manage_appartment_facility.name'),
            name: 'name',
            data: 'name'
        },
        {
            title: Helper.trans('root.appartment_manage_appartment_facility.description'),
            name: 'description',
            data: 'description'
        },
        {
            title: Helper.trans('root.appartment_manage_appartment_facility.icon'),
            name: 'icon',
            data: 'icon'
        },
        {
        ...
        },
        {
            ...
            render: function (data, type, row, col) {
                var url_edit = `admin/appartment-manage/appartment-facility/${data}/edit`;
                var url = `admin/appartment-manage/appartment-facility/${data}`;
                let returnData = ``;
                if (Helper.isShow('appartment_facilities.edit')) {
                ...
                if (Helper.isShow('appartment_facilities.destroy')) {
                    returnData += `<a data-val="${data}" class="btn btn-xs btn-danger waves-effect m-r-5 appartmentFacilityDelete"
        ...
        ajax: {
            url: Helper.siteUrl("admin/appartment-manage/appartment-facility"),
        },
        language: {
            paginate: {
                next: '&#8594;', // or '→'
    ...
    $(document).on('click', '.appartmentFacilityDelete', function () {
        var el = $(this);
        let config = Object.assign(Helper.swalConfig());
        Swal.fire(config).then(function (result) {
            if (result.value === true) {
                Helper.delete('admin/appartment-manage/appartment-facility/' + el.attr('data-val'), '#appartmentFacilityDataTable');
            }
        });
    });
    ...
    $(document).on('click', '.custome-switch', function () {
        var el = $(this);
        Helper.updateStatus('admin/appartment-manage/appartment-facility/update-status/' + el.attr('data-val'));
    });

    // $("#appartmentFacilityDataTable tbody").sortable({
    //     items: "tr",
    //     cursor: 'move',
    //     opacity: 0.6,
    //     update: function() {
    //         sendOrderToServer('#appartmentFacilityDataTable', 'admin/slider-sortable', appartmentFacilityDataTable.page
    //             .info().page);
    //     }
    // });
