...
$(document).ready(function () {
    var appartmentTypeDataTable = $('#appartmentTypeDataTable').DataTable({
        ...
        columns: [{
            ...
            render: function (data, row, type, col) {
                var pageInfo = appartmentTypeDataTable.page.info();
                return (col.row + 1) + pageInfo.start;
            }
        },
        {
            title: Helper.trans('root.appartment_manage_appartment_type.name'),
            name: 'name',
            data: 'name'
        },
        {
            title: Helper.trans('root.appartment_manage_appartment_type.description'),
            name: 'description',
            data: 'description'
        },
        {
        ...
        },
        {
            ...
            render: function (data, type, row, col) {
                var url_edit = `admin/appartment-manage/appartment-type/${data}/edit`;
                var url = `admin/appartment-manage/appartment-type/${data}`;
                let returnData = ``;
                if (Helper.isShow('appartment_types.edit')) {
                ...
                if (Helper.isShow('appartment_types.destroy')) {
                    returnData += `<a data-val="${data}" class="btn btn-xs btn-danger waves-effect m-r-5 deleteAppartmentType"
        ...
        ajax: {
            url: Helper.siteUrl("admin/appartment-manage/appartment-type"),
        },
        language: {
            paginate: {
                next: '&#8594;', // or '→'
    ...
    $(document).on('click', '.deleteAppartmentType', function () {
        var el = $(this);
        let config = Object.assign(Helper.swalConfig());
        Swal.fire(config).then(function (result) {
            if (result.value === true) {
                Helper.delete('admin/appartment-manage/appartment-type/' + el.attr('data-val'), '#appartmentTypeDataTable');
            }
        });
    });
    ...
    $(document).on('click', '.custome-switch', function () {
        var el = $(this);
        Helper.updateStatus('admin/appartment-manage/appartment-type/update-status/' + el.attr('data-val'));
    });
    
    // $("#appartmentTypeDataTable tbody").sortable({
    //     items: "tr",
    //     cursor: 'move',
    //     opacity: 0.6,
    //     update: function() {
    //         sendOrderToServer('#appartmentTypeDataTable', 'admin/slider-sortable', appartmentTypeDataTable.page
    //             .info().page);
    //     }
    // });
