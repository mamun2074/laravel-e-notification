...
$(document).ready(function () {
    var pageDataTable = $('#pageDataTable').DataTable({
        ...
        columns: [{
            ...
            render: function (data, row, type, col) {
                var pageInfo = pageDataTable.page.info();
                return (col.row + 1) + pageInfo.start;
            }
        },
        {
            title: Helper.trans('root.page.name'),
            name: 'name',
            data: 'name'
        },
        {
            title: Helper.trans('root.page.slug'),
            name: 'slug',
            data: 'slug'
        },
        {
            title: Helper.trans('root.page.page_title'),
            name: 'page_title',
            data: 'page_title'
        },
        {
            title: Helper.trans('root.page.banner_bg_path'),
            name: 'banner_bg_path',
            data: 'banner_bg_path',
            render: (data) => (data) ? `<img width="100px" src="${Helper.getStorageUrl(data)}" alt="">` : ''
        },
        {
        ...
        },
        {
            ...
            render: function (data, type, row, col) {
                var url_edit = `admin/page-manage/page/${data}/edit`;
                var url = `admin/page-manage/page/${data}`;
                let returnData = ``;
                if (Helper.isShow('pages.edit')) {
                ...
                if (Helper.isShow('pages.destroy')) {
                    if (row['is_static'] == 1) {
                        returnData += `<a data-val="${data}" class="btn btn-xs btn-danger waves-effect m-r-5 pageDelete"
        ...
        ajax: {
            url: Helper.siteUrl("admin/page-manage/page"),
        },
        language: {
            paginate: {
                next: '&#8594;', // or '→'
    ...
    $(document).on('click', '.pageDelete', function () {
        ...
        Swal.fire(config).then(function (result) {
            if (result.value === true) {
                Helper.delete('admin/page-manage/page/' + el.attr('data-val'), '#pageDataTable');
            }
        });
    });
    ...
    $(document).on('click', '.custome-switch', function () {
        var el = $(this);
        Helper.updateStatus('admin/page-manage/page/update-status/' + el.attr('data-val'));
    });

    // $("#pageDataTable tbody").sortable({
    //     items: "tr",
    //     cursor: 'move',
    //     opacity: 0.6,
    //     update: function() {
    //         sendOrderToServer('#pageDataTable', 'admin/slider-sortable', pageDataTable.page
    //             .info().page);
    //     }
    // });
