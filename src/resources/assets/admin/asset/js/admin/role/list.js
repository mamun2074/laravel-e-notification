...
$(document).ready(function () {
    var roleDataTable = $('#roleDataTable').DataTable({
        ...
        buttons: Helper.datatableButtons([0, 1, 3], 'role'),
        ...
        {
            title: Helper.trans('root.general_settings_role.name'),
            name: 'name',
            data: 'name'
        },
        {
        ...
        },
        {
            ...
            render: function (data, type, row, col) {
                var url_edit = `admin/user-manage/role/${data}/edit`;
                var url = `admin/user-manage/role/${data}`;

                let returnData = ``;
                if (Helper.isShow('roles.edit')) {
                ...
                if (Helper.isShow('roles.destroy')) {
                    returnData += `<a data-val="${data}" class="btn btn-xs btn-danger waves-effect m-r-5 deleteRole"
        ...
        ajax: {
            url: Helper.siteUrl("admin/user-manage/role"),
        },
        language: {
            paginate: {
                next: '&#8594;', // or '→'
    ...
    $(document).on('click', '.deleteRole', function () {
        ...
        Swal.fire(config).then(function (result) {
            if (result.value === true) {
                Helper.delete('admin/user-manage/role/' + el.attr('data-val'), '#roleDataTable');
            }
        });
    });
    ...
    $(document).on('click', '.custome-switch', function () {
        var el = $(this);
        Helper.updateStatus('admin/user-manage/role/update-status/' + el.attr('data-val'));
    });

    // $("#roleDataTable tbody").sortable({
    //     items: "tr",
    //     cursor: 'move',
    //     opacity: 0.6,
    //     update: function() {
    //         sendOrderToServer('#roleDataTable', 'admin/slider-sortable', roleDataTable.page
    //             .info().page);
    //     }
    // });
