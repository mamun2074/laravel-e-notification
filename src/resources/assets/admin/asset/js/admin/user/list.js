...
$(document).ready(function () {
    var userDataTable = $('#userDataTable').DataTable({
        ...
        buttons: Helper.datatableButtons([0, 1, 2, 3], 'user'),
        ...
        {
            title: Helper.trans('root.user.name'),
            name: 'name',
            data: 'name'
        },
        {
            title: Helper.trans('root.user.email'),
            name: 'email',
            data: 'email'
        },
        {
            title: Helper.trans('root.user.user_type'),
            name: 'user_type',
            data: 'user_type',
        ...
        {
            title: Helper.trans('root.user.roles'),
            name: 'roles',
            data: 'roles',
            width: '100px',
            class: 'text-center',
            render: function (data, type, row, col) {
                ...
                $.each(data, function (index, item) {
                    if (item['name'] !== 'super-admin') {
                        role_names += `<span class="badge bg-cyan m-l-5">${item['name']}</span>`;
                    }
                });
                return role_names;
            },
        ...
        },
        {
            ...
            render: function (data, type, row, col) {
                var url_edit = `admin/user-manage/user/${data}/edit`;
                var url = `admin/user-manage/user/${data}`;
                var user_profile_url = `admin/user-manage/user/profile/${data}`;
                let returnData = ``;
                if (Helper.isShow('users.edit')) {
                ...
                if (Helper.isShow('users.destroy')) {
                    returnData += `<a data-val="${data}" class="btn btn-xs btn-danger waves-effect m-r-5 deleteUser"
                                href="javascript:void(0);" data-toggle="tooltip" data-placement="top" title="Delete">
                                    <i class="material-icons">delete</i>
                                    </a>`;

                }
                if (Helper.isShow('users.profile_update')) {
                    returnData += `<a class="btn btn-xs btn-warning waves-effect m-r-5"
                                    href="${Helper.siteUrl(user_profile_url)}" data-toggle="tooltip" data-placement="top" title="Profile Edit">
        ...
        ajax: {
            url: Helper.siteUrl("admin/user-manage/user"),
        },
        language: {
            paginate: {
                next: '&#8594;', // or '→'
    ...
    $(document).on('click', '.deleteUser', function () {
        ...
        Swal.fire(config).then(function (result) {
            if (result.value === true) {
                Helper.delete('admin/user-manage/user/' + el.attr('data-val'), '#userDataTable');
            }
        });
    });
    ...
    $(document).on('click', '.custome-switch', function () {
        var el = $(this);
        Helper.updateStatus('admin/user-manage/user/update-status/' + el.attr('data-val'));
    });

    // $("#userDataTable tbody").sortable({
    //     items: "tr",
    //     cursor: 'move',
    //     opacity: 0.6,
    //     update: function() {
    //         sendOrderToServer('#userDataTable', 'admin/slider-sortable', userDataTable.page
    //             .info().page);
    //     }
    // });
