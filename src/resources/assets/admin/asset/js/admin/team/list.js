...
$(document).ready(function () {
    var teamDataTable = $('#teamDataTable').DataTable({
        ...
        buttons: Helper.datatableButtons([0, 1, 2, 5], 'team'),
        ...
        {
            title: Helper.trans('root.general_settings_team.name'),
            name: 'name',
            data: 'name'
        },
        {
            title: Helper.trans('root.general_settings_team.designation'),
            name: 'designation',
            data: 'designation'
        },
        {
            title: Helper.trans('root.general_settings_team.photo_path'),
            name: 'photo_path',
            data: 'photo_path',
            render: (data) => (data) ? `<img width="50px" src="${Helper.getStorageUrl(data)}" alt="">` : ''
        },
        {
            title: Helper.trans('root.general_settings_team.fb_url'),
            name: 'fb_url',
            data: 'fb_url'
        },
        {
            title: Helper.trans('root.general_settings_team.twitter_url'),
            name: 'twitter_url',
            data: 'twitter_url'
        },
        {
            title: Helper.trans('root.general_settings_team.linkedin_url'),
            name: 'linkedin_url',
            data: 'linkedin_url'
        },
        {
            title: Helper.trans('root.general_settings_team.instagram_url'),
            name: 'instagram_url',
            data: 'instagram_url'
        },
        {
        ...
        },
        {
            ...
            render: function (data, type, row, col) {
                var url_edit = `admin/general-settings/team/${data}/edit`;
                var url = `admin/general-settings/team/${data}`;
                let returnData = ``;
                if (Helper.isShow('teams.edit')) {
                ...
                if (Helper.isShow('teams.destroy')) {

                    returnData += `<a data-val="${data}" class="btn btn-xs btn-danger waves-effect m-r-5 teamDelete"
        ...
        ajax: {
            url: Helper.siteUrl("admin/general-settings/team"),
        },
        language: {
            paginate: {
                next: '&#8594;', // or '→'
    ...
    $(document).on('click', '.teamDelete', function () {
        ...
        Swal.fire(config).then(function (result) {
            if (result.value === true) {
                Helper.delete('admin/general-settings/team/' + el.attr('data-val'), '#teamDataTable');
            }
        });
    });
    ...
    $(document).on('click', '.custome-switch', function () {
        var el = $(this);
        Helper.updateStatus('admin/general-settings/team/update-status/' + el.attr('data-val'));
    });

    // $("#teamDataTable tbody").sortable({
    //     items: "tr",
    //     cursor: 'move',
    //     opacity: 0.6,
    //     update: function() {
    //         sendOrderToServer('#teamDataTable', 'admin/slider-sortable', teamDataTable.page
    //             .info().page);
    //     }
    // });
