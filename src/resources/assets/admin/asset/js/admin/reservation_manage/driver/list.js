...
$(document).ready(function () {
    var driverDataTable = $('#driverDataTable').DataTable({
        ...
        buttons: Helper.datatableButtons([0, 1, 2, 3, 4], 'driver'),
        ...
        columns: [{
            ...
            render: function (data, row, type, col) {
                var pageInfo = driverDataTable.page.info();
                return (col.row + 1) + pageInfo.start;
            }
        },
        {
            title: Helper.trans('root.reservation_manage_driver.name'),
            name: 'name',
            data: 'name'
        },
        {
            title: Helper.trans('root.reservation_manage_driver.email'),
            name: 'email',
            data: 'email'
        },
        {
            title: Helper.trans('root.reservation_manage_driver.car_name'),
            name: 'car',
            data: 'car',
            width: '100px',
            class: 'text-left',
        ...
        },
        {
            ...
            render: function (data, type, row, col) {
                var url_edit = `admin/reservation-manage/driver/${data}/edit`;
                var url = `admin/reservation-manage/driver/${data}`;
                let returnData = ``;
                ...
                if (Helper.isShow('drivers.destroy')) {
                    returnData += `<a data-val="${data}" class="btn btn-xs btn-danger waves-effect m-r-5 driverDelete"
        ...
        ajax: {
            url: Helper.siteUrl("admin/reservation-manage/driver"),
        },
        language: {
            paginate: {
                next: '&#8594;', // or '→'
    ...
    $(document).on('click', '.driverDelete', function () {
        ...
        Swal.fire(config).then(function (result) {
            if (result.value === true) {
                Helper.delete('admin/reservation-manage/driver/' + el.attr('data-val'), '#driverDataTable');
            }
        });
    });
    ...
    $(document).on('click', '.custome-switch', function () {
        var el = $(this);
        Helper.updateStatus('admin/reservation-manage/driver/update-status/' + el.attr('data-val'));
    });

    // $("#driverDataTable tbody").sortable({
    //     items: "tr",
    //     cursor: 'move',
    //     opacity: 0.6,
    //     update: function() {
    //         sendOrderToServer('#driverDataTable', 'admin/slider-sortable', driverDataTable.page
    //             .info().page);
    //     }
    // });
