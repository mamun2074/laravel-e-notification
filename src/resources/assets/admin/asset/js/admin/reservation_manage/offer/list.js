...
$(document).ready(function () {
    var offerDataTable = $('#offerDataTable').DataTable({
        ...
        buttons: Helper.datatableButtons([0, 1, 2, 3, 4, 5, 6, 7], 'Offer'),
        ...
        columns: [{
            ...
            render: function (data, row, type, col) {
                var pageInfo = offerDataTable.page.info();
                return (col.row + 1) + pageInfo.start;
            }
        },
        {
            title: Helper.trans('root.reservation_manage_offer.name'),
            name: 'name',
            data: 'name'
        },
        {
            title: Helper.trans('root.reservation_manage_offer.description'),
            name: 'description',
            data: 'description'
        },
        {
            title: Helper.trans('root.reservation_manage_offer.offer_type'),
            name: 'offer_type',
            data: 'offer_type',
            render: function (data, type, row, col) {
                var rateType = (data == 1) ? `Percentage` : `Fixed`;
                return rateType;
            }
        },
        {
            title: Helper.trans('root.reservation_manage_offer.offer_amount'),
            name: 'offer_amount',
            data: 'offer_amount'
        },
        {
            title: Helper.trans('root.reservation_manage_offer.start_date'),
            name: 'start_date',
            data: 'start_date'
        },
        {
            title: Helper.trans('root.reservation_manage_offer.end_date'),
            name: 'end_date',
            data: 'end_date'
        },
        {
        ...
        },
        {
            ...
            render: function (data, type, row, col) {
                var url_edit = `admin/reservation-manage/offer/${data}/edit`;
                var url = `admin/reservation-manage/offer/${data}`;
                let returnData = ``;
                if (Helper.isShow('offers.edit')) {
                ...
                if (Helper.isShow('offers.destroy')) {
                    returnData += `<a data-val="${data}" class="btn btn-xs btn-danger waves-effect m-r-5 offerDelete"
        ...
        ajax: {
            url: Helper.siteUrl("admin/reservation-manage/offer"),
        },
        language: {
            paginate: {
                next: '&#8594;', // or '→'
    ...
    $(document).on('click', '.offerDelete', function () {
        ...
        Swal.fire(config).then(function (result) {
            if (result.value === true) {
                Helper.delete('admin/reservation-manage/offer/' + el.attr('data-val'), '#offerDataTable');
            }
        });
    });
    ...
    $(document).on('click', '.custome-switch', function () {
        var el = $(this);
        Helper.updateStatus('admin/reservation-manage/offer/update-status/' + el.attr('data-val'));
    });

    // $("#offerDataTable tbody").sortable({
    //     items: "tr",
    //     cursor: 'move',
    //     opacity: 0.6,
    //     update: function() {
    //         sendOrderToServer('#offerDataTable', 'admin/slider-sortable', offerDataTable.page
    //             .info().page);
    //     }
    // });
