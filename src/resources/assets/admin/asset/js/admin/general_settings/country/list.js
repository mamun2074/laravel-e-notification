...
$(document).ready(function () {
    var countryDataTable = $('#countryDataTable').DataTable({
        ...
        {
            title: Helper.trans('root.general_settings_country.name'),
            name: 'name',
            data: 'name'
        },
        {
            title: Helper.trans('root.general_settings_country.currency_code'),
            name: 'currency_code',
            data: 'currency_code'
        },
        {
            title: Helper.trans('root.general_settings_country.currency_symbol'),
        ...
        },
        {
            ...
            render: function (data, type, row, col) {
                var url_edit = `admin/general-settings/country/${data}/edit`;
                var url = `admin/general-settings/country/${data}`;
                let returnData = ``;
                if (Helper.isShow('countries.edit')) {
                ...
                if (Helper.isShow('countries.destroy')) {
        ...
        ajax: {
            url: Helper.siteUrl("admin/general-settings/country"),
        },
        language: {
            paginate: {
                next: '&#8594;', // or '→'
    ...
    $(document).on('click', '.deleteCountry', function () {
        ...
        Swal.fire(config).then(function (result) {
            if (result.value === true) {
                Helper.delete('admin/general-settings/country/' + el.attr('data-val'), '#countryDataTable');
            }
        });
    });
    ...
    $(document).on('click', '.custome-switch', function () {
        var el = $(this);
        Helper.updateStatus('admin/general-settings/country/update-status/' + el.attr('data-val'));
    });

    // $("#countryDataTable tbody").sortable({
    //     items: "tr",
    //     cursor: 'move',
    //     opacity: 0.6,
    //     update: function() {
    //         sendOrderToServer('#countryDataTable', 'admin/slider-sortable', countryDataTable.page
    //             .info().page);
    //     }
    // });
